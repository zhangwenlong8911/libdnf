...
TransactionCB::TransactionCB(sdbus::IProxy * proxy, std::string session_object_path)
    ...
    proxy->registerSignalHandler(
        dnfdaemon::INTERFACE_RPM, dnfdaemon::SIGNAL_TRANSACTION_TRANSACTION_START, [this](sdbus::Signal & signal) -> void {
            this->transaction_start(signal);
        });
    proxy->registerSignalHandler(
        dnfdaemon::INTERFACE_RPM, dnfdaemon::SIGNAL_TRANSACTION_TRANSACTION_PROGRESS, [this](sdbus::Signal & signal) -> void {
            this->transaction_progress(signal);
        });
    proxy->registerSignalHandler(
...
void TransactionCB::new_progress_bar(uint64_t total, const std::string & description) {
    ...
    auto progress_bar = new libdnf::cli::progressbar::DownloadProgressBar(static_cast<int64_t>(total), description);
    multi_progress_bar.add_bar(progress_bar);
    progress_bar->set_auto_finish(false);
    progress_bar->start();
